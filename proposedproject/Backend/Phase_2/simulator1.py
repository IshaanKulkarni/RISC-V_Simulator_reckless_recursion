...
def memory_op(instruction_word, instruction_line,adv):
    if instruction_word=="sw":
        result=RAM[int(registers[instruction_line[1][2:]])-int(BaseAddress[2:])+adv]=int(registers[instruction_line[0]])
    elif instruction_word=="sw":
        result=RAM[int(registers[instruction_line[1][2:]])-int(BaseAddress[2:])+adv]
    return result

def write_back_op(instr_line, result):
    try:
        registers[instr_line[0]]=result
        return 1
    except:
        return -1
...
def add(instruction_line):
    ...
    if isinstance(registers[instruction_line[1]],int) and isinstance(registers[instruction_line[2]],int):
        ...
        return -1,instruction_line
    return registers[instruction_line[0]],instruction_line
...
def lw(instruction_line):
    ...
    return forward,instruction_line
...
def li(instruction_line):
    ...
    return int(instruction_line[1]),instruction_line
...
def find_typeof(line):
    instruction_line=""
    instruction_word=line.split(sep=" ",maxsplit=1)

    try:
        instruction_line=instruction_word[1]
    except:
        pass
    instruction_word=instruction_word[0]

    return instruction_word,instruction_line

# Excetuing the instructions in the file using if else ladder
def execute_instructions(instruction_word,instruction_line):
    # print("line ",line)
    ...
    if instruction_word=="add":
        return add(instruction_line)
    elif instruction_word=="sub":
        return sub(instruction_line)
    elif instruction_word in ("bne","beq","j"):
        return 0,0
    if instruction_word=="lw":
        return lw(instruction_line)
    if instruction_word=="sw":
        return sw(instruction_line)
    if instruction_word=="li":
        return li(instruction_line)
    if instruction_word=="mul":
        return mul(instruction_line)

    if instruction_word=="sll":
...
def reckless():
    # sanitize()
    global PC
    PC = find_typeof(lines[line_counter])
